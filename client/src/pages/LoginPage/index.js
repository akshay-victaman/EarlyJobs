...
const LoginPage = () => {
  ...
  const [loginType, setLoginType] = useState(
    loginTypeFromLocalStorage ? loginTypeFromLocalStorage : loginTypes.user
  );
      ...
      <div className="homepage-sub-con">
        <div className="homepage-card">
          <div className="login-type-con">
            <button
              className={`login-type-button ${
                loginTypes.user === loginType ? "active-login-btn" : ""
              }`}
              onClick={() => handleLoginTypeChange(loginTypes.user)}
            >
              Recruiter Login
            </button>
            <button
              className={`login-type-button ${
                loginTypes.college === loginType ? "active-login-btn" : ""
              }`}
              onClick={() => handleLoginTypeChange(loginTypes.college)}
            >
              College Login
            </button>
            <button
              className={`login-type-button ${
                loginTypes.agency === loginType ? "active-login-btn" : ""
              }`}
              onClick={() => handleLoginTypeChange(loginTypes.agency)}
          ...
          <form onSubmit={handleLogin} className="login-form">
            ...
            <label className="homepage-label" id="email">
              EMAIL
            </label>
            <input
              type="email"
            ...
            />
            <label className="homepage-label" id="password">
              PASSWORD
            </label>
            <input
              type="password"
            ...
            <button
              type="submit"
              disabled={loading}
              className="login-button login-loader"
            >
              {loading ? (
                <Oval
                  visible={true}
