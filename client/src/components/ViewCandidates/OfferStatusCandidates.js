...
const OfferStatusCandidates = ({
  ...
  else if (showCandidateForm === 6 || showCandidateForm === 12)
    offerStatus = "Joined";
        ...
        if (data.error) {
          ...
          const updatedData = data.candidates.map((eachItem) => ({
            ...
            interviewDate: eachItem.interview_date
              ? formatDate(eachItem.interview_date)
              : null,
            offeredDate: eachItem.offered_date
              ? formatDate(eachItem.offered_date)
              : null,
            jobId: eachItem.job_id,
            hrName: eachItem.hr_name,
            dayCount: eachItem.offered_date
              ? calculateDayCount(
                  eachItem.offered_date,
                  eachItem.tenure_in_days
                )
              : null,
          ...
          const updatedData = data.map((eachItem) => ({
            ...
            commissionPayable:
              showCandidateForm === 12 && Cookies.get("role") === "SHM"
                ? eachItem.commission_paid
                : null,
            commissionClaimed:
              showCandidateForm === 12 &&
              (Cookies.get("role") === "AC" || Cookies.get("role") === "SHM")
                ? eachItem.is_claimed === 1
                  ? eachItem.commission_paid
                  : 0
                : null,
          }));
        ...
        if (data.error) {
          ...
          const updatedData = data.map((eachItem) => ({
            ...
            dateOfBirth: eachItem.date_of_birth
              ? formatDate(eachItem.date_of_birth).substring(0, 11)
              : null,
            companyName: eachItem.company_name,
  ...
  const deleteEmploymentDetails = async (applicationId) => {
    try {
      ...
      console.error(
        "Error occurred while submitting employment details: ",
        error
      );
    }
    setLoading(false);
  ...
  return (
    <div
      style={{ width: "100%" }}
      className="job-details-candidates-container jobs-section-candidate-container"
    >
      <h1 className="bde-heading" style={{ textAlign: "center" }}>
        <span className="head-span">
          {showCandidateForm === 12
            ? "Tenure Status"
            : `${offerStatus} Candidates`}
        </span>
      </h1>
      ...
      <div className="job-section-select-filter-container">
        <div className="job-section-select-container">
          <label
            className="homepage-label view-candidates-label"
            htmlFor="resume"
          >
            Select Job
          </label>
          <select
            className="homepage-input view-candidates-select"
            name="jobId"
            id="jobId"
            value={jobId}
        ...
        {Cookies.get("role") === "BDE" && showCandidateForm === 6 && (
          <div className="job-section-select-container">
            <label
              className="homepage-label view-candidates-label"
              htmlFor="handleVerificationStatusChange"
            >
              Filter By Verification Status
            </label>
            <select
              className="homepage-input view-candidates-select"
              name="handleVerificationStatusChange"
              id="handleVerificationStatusChange"
        ...
        {(Cookies.get("role") === "AC" || Cookies.get("role") === "SHM") &&
          showCandidateForm === 12 && (
            <div className="job-section-select-container">
              <label
                className="homepage-label view-candidates-label"
                htmlFor="handleClamiedStatusChange"
              >
                Filter By Claimed Status
              </label>
              <select
                className="homepage-input view-candidates-select"
                name="handleClamiedStatusChange"
                id="handleClamiedStatusChange"
                value={clamiedStatus}
                onChange={handleClamiedStatusChange}
              >
        ...
        {((Cookies.get("role") === "BDE" && showCandidateForm === 6) ||
          ...
          <>
            <div className="job-section-select-container">
              <label
                className="homepage-label view-candidates-label"
                htmlFor="handleTenureStatusChange"
              >
                Filter By Tenure Status
              </label>
              <select
                className="homepage-input view-candidates-select"
                name="handleTenureStatusChange"
                id="handleTenureStatusChange"
                value={tenureStatus}
            ...
            <div className="job-section-select-container">
              <label
                className="homepage-label view-candidates-label"
                htmlFor="handleApproveStatusChange"
              >
                Filter By Approve Status
              </label>
              <select
                className="homepage-input view-candidates-select"
                name="handleApproveStatusChange"
                id="handleApproveStatusChange"
                value={approveStatus}
        ...
        <div className="job-section-select-container">
          ...
          >
            ...
            {showCandidateForm === 5
              ? "Selection Date"
              : showCandidateForm === 6 || showCandidateForm === 12
              ? "Joining Date"
              : "Interview Date"}{" "}
            (From - To)
          </label>
          <div className="date-con">
            <input
              className="homepage-input view-candidates-select interview-date-input"
              type="date"
              id="interview-date"
              value={fromDate}
        ...
        <div className="user-view-search-con my-hr-recruiters-search-con view-candidates-search-input-con">
          <input
            className="user-view-search-input my-hr-recruiter-search-input"
            type="search"
            value={searchInput}
          ...
          />
          <div
            className="user-view-search-button my-hr-recruiters-search-btn"
            onClick={
        ...
        {showCandidateForm === 6 && (
          <>
            <div className="rows-count-con">
              ...
              <span className="rows-count-number">
                `
                {verificationCount.verified_count
                  ? verificationCount.verified_count
                  : 0}
                `
              </span>
            ...
            <div className="rows-count-con">
              ...
              <span className="rows-count-number">
                `
                {verificationCount.not_verified_count
                  ? verificationCount.not_verified_count
                  : 0}
                `
              </span>
            ...
            <div className="rows-count-con">
              ...
              <span className="rows-count-number">
                `
                {verificationCount.unknown_count
                  ? verificationCount.unknown_count
                  : 0}
                `
              </span>
          ...
          (Cookies.get("role") === "AC" || Cookies.get("role") === "SHM") && (
            <>
              <div className="rows-count-con">
                ...
                <span className="rows-count-number">
                  `
                  {verificationCount.claimed_count
                    ? verificationCount.claimed_count
                    : 0}
                  `
                </span>
              ...
              <div className="rows-count-con">
                ...
                <span className="rows-count-number">
                  `
                  {verificationCount.not_claimed_count
                    ? verificationCount.not_claimed_count
                    : 0}
                  `
                </span>
              ...
              <div className="rows-count-con">
                <span className="rows-count-text">
                  Total{" "}
                  {Cookies.get("role") === "AC" ? "Receivable" : "Payable"}:
                </span>
                <span className="rows-count-number">
                  `
                  {verificationCount.total_unclaimed_amount
                    ? verificationCount.total_unclaimed_amount
                    : 0}
                  `
                </span>
              ...
              <div className="rows-count-con">
                ...
                <span className="rows-count-number">
                  `
                  {verificationCount.total_claimed_amount
                    ? verificationCount.total_claimed_amount
                    : 0}
                  `
                </span>
      ...
      <div className="table-candidate-container">
        <table
          className={`job-details-candidates-table candidate-table-job-section ${
            candidateList.length === 0 ? "empty-candidates" : ""
          }`}
        >
          <tr className="job-details-candidates-table-heading">
            ...
            {(showCandidateForm === 12 ||
              (Cookies.get("role") === "BDE" && showCandidateForm === 6)) && (
              <th className="job-details-candidates-table-heading-cell">
                Tenure (Days Left / Status / Approve)
              </th>
            )}
            {Cookies.get("role") === "BDE" && showCandidateForm === 6 && (
              <th className="job-details-candidates-table-heading-cell">
                Approve Tenure Status
              </th>
            )}
          ...
          {candidateList.length > 0 &&
            candidateList.map((eachItem) => {
              ...
              return (
                <tr
                  key={eachItem.email}
                  className="job-details-candidates-table-row"
                >
                  <td
                    className="job-details-candidates-table-cell job-details-candidates-table-cell-hover"
                    onClick={() => onShowCandidateDetails(eachItem.candidateId)}
                  >
                    {eachItem.name}
                  </td>
                  ...
                  {showCandidateForm !== 7 && (
                    <td className="job-details-candidates-table-cell">
                      ...
                      showCandidateForm === 11
                        ...
                        : eachItem.offeredDate !== null
                        ? eachItem.offeredDate.slice(0, -8)
                        : "Null"}
                      {showCandidateForm === 11 && (
                        <button
                          type="button"
                          className="shedule-interview-button"
                          onClick={() =>
                            onShowScheduleInterviewPopup(
                              jobId1,
                              { ...eachItem, candidateName: eachItem.name },
                              allJobsList,
                              setCandidateList,
                              candidateList
                            )
                  ...
                  <td className="job-details-candidates-table-cell">
                    {eachItem.employeeId ? eachItem.employeeId : "--"}
                  </td>
                  ...
                  {(showCandidateForm === 12 ||
                    ...
                    <td className="job-details-candidates-table-cell">
                      ...
                      {eachItem.tenureStatus !== null &&
                        " / " + eachItem.tenureStatus}
                      {eachItem.isTenureApproved !== null &&
                        " / " + eachItem.isTenureApproved}
                    </td>
                  )}
                  {Cookies.get("role") === "BDE" && showCandidateForm === 6 && (
                    <td className="job-details-candidates-table-cell">
                      {!loading ? (
                        ...
                        eachItem.tenureStatus === null ? (
                          <ApproveTenureStatus
                            candidate={eachItem}
                            onUpdate={updateTenureApprovalStatus}
                          />
                        ) : (
                          "--"
                        )
                  ...
                  {(showCandidateForm === 5 || showCandidateForm === 6) && (
                    <td className="job-details-candidates-table-cell">
                      {eachItem.verificationStatus !== null
                        ? eachItem.verificationStatus
                        : "--"}
                    </td>
                  )}
                  {(showCandidateForm === 12 ||
                  ...
                  {showCandidateForm === 12 &&
                    (Cookies.get("role") === "AC" ||
                      ...
                      <>
                        ...
                        <td className="job-details-candidates-table-cell">
                          {eachItem.commission_paid === null
                            ? "--"
                            : eachItem.isClaimed === 1
                            ? eachItem.commission_paid
                            : 0}
                        </td>
                      </>
        ...
        {candidateList.length === 0 && (
          <p className="no-candidates-error">{renderNoCandidates()}</p>
        )}
      </div>
      ...
      {showTenureApprovedPopUp && (
        <TenureApprovedPopUp
          setShowTenureApprovedPopUp={setShowTenureApprovedPopUp}
