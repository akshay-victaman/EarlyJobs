...
const UploadCandidatePage = ({ setShowCandidateForm }) => {
  ...
  const onAddSkills = () => {
    ...
    setCandidateDetails({
      ...candidateDetails,
      skills: [...candidateDetails.skills, trimmedSkills],
    });
    setSkills("");
  };
  ...
  const onRemoveSkills = (id) => {
    setCandidateDetails({
      ...candidateDetails,
      skills: candidateDetails.skills.filter((skill, index) => index !== id),
    });
  };
  ...
  const getHrAssignedHm = async () => {
    const url = `${backendUrl}/api/users/hr-assigned-hm/${Cookies.get(
      "email"
    ...
    if (response.ok === true) {
      if (data.error) {
        ...
        const updatedData = data.map((eachItem) => ({
          ...
          locationLink: eachItem.location_link,
          city: eachItem.city,
          area: eachItem.area,
        }));
  ...
  const sendEmailAck = async () => {
    ...
    const jobName = jobsList.find(
      (job) => job.id === candidateDetails.jobId
    ).role;
    const companyName = jobsList.find(
      (job) => job.id === candidateDetails.jobId
    ).compname;
    const location = jobsList.find(
      (job) => job.id === candidateDetails.jobId
    ).location;
    // const interviewDateTime = new Date(`${candidateDetails.interviewDate}T${candidateDetails.interviewTime}`);
    const interviewDateTime = parse(
      `${candidateDetails.interviewDate} ${candidateDetails.interviewTime}`,
      "yyyy-M-d HH:mm",
      new Date()
    );
    ...
    let emailContent = `
            ...
            If you need any help, please coordinate with ${
              role === "SHM"
                ? `${username} Victaman, at ${hmHrData.shm[0].phone}, ${hmHrData.shm[0].email} or Earlyjobs call/wtsp: 8217527926`
                : role === "AC"
                ? `${username} Victaman, at ${hmHrData.hm[0].phone}, ${hmHrData.hm[0].email} or ${hmHrData.shm[0].username} Victaman, at ${hmHrData.shm[0].phone}, ${hmHrData.shm[0].email} or Earlyjobs call/wtsp: 8217527926`
                : `${username} Victaman, at ${hmHrData.hr[0].phone}, ${hmHrData.hr[0].email} or ${hmHrData.hm[0].username}, Victaman at ${hmHrData.hm[0].phone}, ${hmHrData.hm[0].email} or Earlyjobs call/wtsp: 8217527926`
            }.
            <br>
            <br>
            Regards,
  ...
  const sendEmailDayBeforeRem = async () => {
    ...
    const interviewDateTime = new Date(
      `${candidateDetails.interviewDate}T${candidateDetails.interviewTime}`
    );
    const yesterday = sub(interviewDateTime, { days: 1 });
    const yesterdayDate = `${yesterday.getFullYear()}-${String(
    ...
    let emailContent = `
            ...
            If you need any help, please coordinate with ${
              role === "SHM"
                ? `${username} Victaman, at ${hmHrData.shm[0].phone}, ${hmHrData.shm[0].email}`
                : role === "AC"
                ? `${username} Victaman, at ${hmHrData.hm[0].phone}, ${hmHrData.hm[0].email} or ${hmHrData.shm[0].username} Victaman, at ${hmHrData.shm[0].phone}, ${hmHrData.shm[0].email}`
                : `${username} Victaman, at ${hmHrData.hr[0].phone}, ${hmHrData.hr[0].email} or ${hmHrData.hm[0].username}, Victaman at ${hmHrData.hm[0].phone}, ${hmHrData.hm[0].email}`
            }.
            <br>
            <br>
            Regards,
  ...
  const sendEmailOnDay = async () => {
    ...
    const queryParameters = {
      ...
      scheduledAt: encodeURIComponent(
        candidateDetails.interviewDate + "07:00:00"
      ),
    };
    const url = `https://enterprise.webaroo.com/GatewayAPI/rest?method=${queryParameters.method}&userid=${queryParameters.userid}&password=${queryParameters.password}&v=${queryParameters.v}&content_type=${queryParameters.contentType}&name=${queryParameters.name}&fromEmailId=${queryParameters.fromEmailId}&subject=${queryParameters.subject}&recipients=${queryParameters.recipients}&content=${queryParameters.content}&replyToEmailID=${queryParameters.replyToEmailID}&scheduled_at=${queryParameters.scheduledAt}`;
  ...
  const sendInterviewEmails = async () => {
    ...
    const interviewDate = `${interviewDateTime.getFullYear()}-${String(
      interviewDateTime.getMonth() + 1
    ).padStart(2, "0")}-${String(interviewDateTime.getDate()).padStart(
      2,
      "0"
    )}`;
    const diff = differenceInDays(
      parseISO(interviewDate),
  ...
  const postCandidateDetails = async (event) => {
    ...
    if (candidateDetails.fullName.trim() === "") {
      ...
      candidateDetails.phone.length < 10 ||
      candidateDetails.phone.length > 10
    ) {
  ...
  const renderSuccessMessage = () => (
    <div className="add-job-container">
      <h1 className="bde-heading-another-job">
        🎉 Successfully Added Candidate Details 🎉
      </h1>
      <div className="upload-candidate-sub-con">
        <button
          className="login-button candidate-button"
          type="button"
          disabled={loading}
          onClick={() => setShowCandidateForm(0)}
        >
          Back
        </button>
        <button
          className="bde-form-btn-an candidate-back-btn"
    ...
    <form className="upload-candidate-form" onSubmit={postCandidateDetails}>
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="candidateName">
            Full Name<span className="hr-form-span"> *</span>
          </label>
          <input
            type="text"
            name="fullName"
        ...
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="fatherName">
            Father Name<span className="hr-form-span"> *</span>
          </label>
          <input
            type="text"
            name="fatherName"
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="candidateEmail">
            Email ID<span className="hr-form-span"> *</span>
          </label>
          <input
            type="text"
            name="email"
        ...
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="candidatePhone">
            Phone Number<span className="hr-form-span"> *</span>
          </label>
          <input
            type="number"
            name="phone"
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="dateOfBirth">
            Date of Birth<span className="hr-form-span"> *</span>
          </label>
          <input
            type="date"
            name="dateOfBirth"
        ...
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="gender">
            Gender
          </label>
          <select
            className="homepage-input"
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="aadharNum">
            Aadhar Number
          </label>
          <input
            type="number"
        ...
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="highestQualification">
            Highest Qualification<span className="hr-form-span"> *</span>
          </label>
          <select
            className="homepage-input"
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="location">
            Current Location<span className="hr-form-span"> *</span>
          </label>
          <input
            type="text"
        ...
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="category">
          ...
          <select
            ...
            id="category"
            onChange={handleCandidateInputChange}
            value={candidateDetails.jobCategory}
            name="jobCategory"
          >
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="experience">
            Experience<span className="hr-form-span"> *</span>
          </label>
          <div className="homepage-input experience-con">
            <input
              ...
              onChange={handleCandidateInputChange}
            />
            <label htmlFor="experienceInYears" className="experience-label">
              Years
            </label>
            <input
              type="number"
              ...
              onChange={handleCandidateInputChange}
            />
            <label htmlFor="experienceInMonths" className="experience-label">
              Months
            </label>
          </div>
        </div>
        ...
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="employment-type">
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="interviewDate">
            Schedule Interveiw Date time<span className="hr-form-span"> *</span>
          </label>
          <div className="interview-input-con homepage-input">
            <input
              type="date"
              name="interviewDate"
              className="homepage-input interview-input"
              id="interviewDate"
              value={candidateDetails.interviewDate}
              onChange={handleCandidateInputChange}
            />
            <input
              type="time"
              ...
              className="homepage-input interview-input"
              id="interviewDate"
              value={candidateDetails.interviewTime}
              onChange={handleCandidateInputChange}
            />
          </div>
        </div>
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="shiftTimings">
          ...
          <select
            ...
            id="shiftTimings"
            onChange={handleCandidateInputChange}
            value={candidateDetails.shiftTimings}
            name="shiftTimings"
          >
      ...
      <div className="upload-candidate-sub-con">
        <div className="upload-candidate-input-con">
          <label className="homepage-label" htmlFor="resume">
            Select Job<span className="hr-form-span"> *</span>
          </label>
          <select
            className="homepage-input"
            name="jobId"
            id="jobId"
            value={candidateDetails.jobId}
            onChange={handleCandidateInputChange}
          >
            <option value="">Select Job</option>
            {jobsList.map((job) => (
              <option key={job.id} value={job.id}>
                {job.role} - {job.compname} - {job.city} - {job.area}
              </option>
            ))}
          </select>
        </div>
        {Cookies.get("role") === "BDE" && (
          <div className="upload-candidate-input-con">
            <label className="homepage-label" htmlFor="joinedDate">
              Joined Date<span className="hr-form-span"> *</span>
            </label>
            <div className="interview-input-con homepage-input">
              <input
                type="date"
                ...
                className="homepage-input interview-input"
                id="joinedDate"
                value={candidateDetails.joinedDate}
                onChange={handleCandidateInputChange}
              />
            </div>
          </div>
  ...
  return (
    <div className="upload-candidate-container">
      <h1 className="bde-heading">
        <span className="head-span">Add Candidate</span>
      </h1>
