...
const RequestJobDetailsPage = () => {
    ...
    if (!querySnap.empty) {
      ...
      const documents = querySnap.docs.map((doc) => {
        ...
        if (!(timestamp instanceof Timestamp)) {
          console.error(
            "AppliedDate is not a valid Timestamp. Skipping conversion."
          );
          return doc.data();
        }
      ...
      const formattedData = {
        ...
        streetAddress: documents[0].streetAddress,
  ...
  const handleRemoveHiringManager = (email) => {
    const hiringManagers = jobDetails.assignedTo.filter(
      (item) => item !== email
    );
    setJobDetails({ ...jobDetails, assignedTo: hiringManagers });
  };
  ...
  const addToFirebase = async () => {
    const db = getFirestore(app);
    const docRef = await addDoc(collection(db, "ApprovedJobVacancies"), {
      firebaseJob,
    }); // add hiring partner details in ApprovedHiringPartners collection
    const docId = docRef.id;
    const approvedDate = new Date();
    await setDoc(doc(db, "ApprovedJobVacancies", docId), {
      ...firebaseJob,
      approvedDate,
      docId,
    }); // update docId in ApprovedHiringPartners collection
    return docId;
  };
  ...
  const onClickReject = async () => {
    ...
    setRejectApproveStatus(true);
    const db = getFirestore(app);
    const docRef = await addDoc(collection(db, "RejectedJobVacancies"), {
      firebaseJob,
    });
    const docId = docRef.id;
    const RejectedDate = new Date();
    await setDoc(doc(db, "RejectedJobVacancies", docId), {
      ...firebaseJob,
      docId,
      RejectedDate,
    });
    ...
    return (
      <div className="modal-form">
        <button
          ...
          disabled={rejectApproveStatus}
          onClick={close}
        >
          &times;
        </button>
        <label className="homepage-label">
          Do you want to reject {firebaseJob.companyName}'s job posting request?
        </label>
        <div className="achieve-button-con">
          <button
            className="job-details-upload-candidate-button"
            disabled={rejectApproveStatus}
            onClick={() => onClickReject(close)}
          >
            YES
          </button>
          <button
    ...
    return (
      <div className="modal-form">
        ...
        <label className="bde-form-label">
          Assign To Account Manager<span className="hr-form-span"> *</span>
        </label>
        ...
        {error && (
          <p className="hr-error" style={{ marginTop: "10px" }}>
            {error}
          </p>
        )}
      </div>
    ...
    return (
      <div className="job-details-container">
        <div className="job-details-card">
          ...
          {(hiringFor === "Freelance HR Recruiter" || userType !== "HR") && (
            <p className="job-detials-misc">
              ...
              {commissionType === "Fixed"
                ? `₹ ${commissionFee} Per Joining`
                : `${commissionFee}% of Annual CTC`}
            </p>
          )}
          {(hiringFor === "Freelance HR Recruiter" || userType !== "HR") && (
            <p className="job-detials-misc">
          ...
          <p className="job-detials-misc">
            <span className="misc-head">Skills:</span> {skillStr}
          </p>
          <p className="job-detials-misc">
          ...
          <div className="job-details-comapany-details-con">
            <h1 className="job-details-company-details-heading">
              Company Details
            </h1>
            ...
            <div className="job-details-company-details-item">
              ...
              <p className="job-details-company-details-item-value">
                {firebaseJob.companyDetails.name}
              </p>
            </div>
            <div className="job-details-company-details-item">
              <p className="job-details-company-details-item-heading">
              ...
              <p className="job-details-company-details-item-value">
                {firebaseJob.companyDetails.email}
              </p>
            </div>
            <div className="job-details-company-details-item">
              <p className="job-details-company-details-item-heading">
          ...
          <div className="hiring-partner-details-button-con">
            <Popup
              trigger={
                <button className="hiring-partner-btn reject-btn">
                  Reject
                </button>
              }
              modal
            >
              {(close) => (
                <div className="modal">{renderRejectPopup(close)}</div>
              )}
            </Popup>
  ...
  return <div className="job-details-main-container">{renderSwitchCase()}</div>;
};
