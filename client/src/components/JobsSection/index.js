...
const JobsSection = ({
  ...
  const getHirignReqCard = async () => {
    ...
    let queryRef;
    if (page === 1) {
      // If it's the first page, no need to use startAfter
      queryRef = query(
      ...
      // If it's not the first page, start after the last document from the previous page
    ...
    <div className="jobs-section-container">
      ...
      <div
        className={`job-section-search-card-con ${
          showCandidateForm === 2 || showCandidateForm === 3
            ? "job-section-view-candidate-con"
            : ""
        } ${showCandidateForm === 1 ? "job-section-candidate" : ""}`}
      >
