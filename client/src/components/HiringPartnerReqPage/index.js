...
const HiringPartnerReqPage = () => {
  ...
  const filteredHrList = hiringPartnerReqList.filter(
    (eachItem) =>
      eachItem.formData.personalDetails.fullName
        .toLowerCase()
        .includes(searchInput.toLowerCase()) ||
      eachItem.formData.personalDetails.email
        .toLowerCase()
        .includes(searchInput.toLowerCase()) ||
      eachItem.formData.personalDetails.phone
        .toLowerCase()
        .includes(searchInput.toLowerCase())
  );
    ...
    <div className="homepage-container">
      <div className="hiring-partner-req-page-content-con">
        ...
        <div className="user-view-search-con" style={{ marginBottom: "20px" }}>
          ...
          <input
            className="user-view-search-input"
            type="search"
            value={searchInput}
          ...
          <table className="users-table hiring-partner-req-table">
            ...
            <tbody>
              {filteredHrList.map((hiringPartnerReq) => {
                return (
                  <tr
                    className="users-table-data-row"
                    key={hiringPartnerReq.formData.docId}
                  >
                    <td data-cell="Name" className="users-table-data">
                      {hiringPartnerReq.formData.personalDetails.fullName}
                    </td>
                    <td data-cell="Email" className="users-table-data">
                      {hiringPartnerReq.formData.personalDetails.email}
                    </td>
                    <td data-cell="Phone" className="users-table-data">
                      {hiringPartnerReq.formData.personalDetails.phone}
                    </td>
                    <td data-cell="Applied Date" className="users-table-data">
                      {hiringPartnerReq.formattedDate}
                    </td>
                    <td data-cell="More Details" className="users-table-data">
                      <Link
                        to={`/admin/recruiter-requests/${hiringPartnerReq.formData.docId}`}
                        className="link"
                      >
                        <button className="hiring-partner-req-btn hrp-button">
                          View
                        </button>
                      </Link>
                    </td>
